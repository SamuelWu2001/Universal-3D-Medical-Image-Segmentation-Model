...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.args.thd: 
            ...
            xd = checkpoint(self.attn, xd)
            xd = checkpoint(self.Depth_Adapter, xd)
            xd = rearrange(xd, '(b n) dh dw c ->(b dh dw) n c', n= hh * ww )

        x = self.norm1(x)
        x = checkpoint(self.attn, x)
        x = checkpoint(self.Space_Adapter, x)
        ...
        x = x + checkpoint(self.mlp, xn) + self.scale * checkpoint(self.MLP_Adapter, x)
        return x
