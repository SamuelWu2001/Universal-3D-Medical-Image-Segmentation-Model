...
'''end'''
unfreeze_layers = ['image_encoder.pos_embed', 
                   ...
                   'prompt_encoder.text_embedding.weight',
                   'prompt_encoder.text_embedding.bias',
                   ]
...
def draw_cal_volume(pred, mask, imgs, case_num, epoch, mode, text, dice_score_list, points=None):
    sigmoid_pred = torch.sigmoid(pred)
    binary_pred = (sigmoid_pred > 0.5).float() 
    mask = (mask > 0.5).float()    
    pred_masks = binary_pred.detach().cpu().numpy()
    gt_masks = mask.detach().cpu().numpy()
    imgs = imgs.detach().cpu().numpy() 
    for i in range(pred_masks.shape[2]):
        pred_mask = pred_masks[:,:,i]
        gt_mask = gt_masks[:,:,i]
        img = imgs[:,:,i]
        # point_loc = points[i].cpu().numpy()
        # xmin = point_loc[0][0] *pred_mask.shape[0] / 256
        # ymin = point_loc[0][1] *pred_mask.shape[0] / 256
        # xmax = point_loc[1][0] *pred_mask.shape[1] / 256
        # ymax = point_loc[1][1] *pred_mask.shape[1] / 256
        if dice_score_list[i] < 0.0001:
            score = 0
        else:
            score = dice_score_list[i]
        # 使用 Matplotlib 绘制图像
        ...
        plt.title('pred_mask')       
        # rect = patches.Rectangle((ymin, xmin), (ymax-ymin), (xmax-xmin), linewidth=2, edgecolor='r', facecolor='none')
        ...
        plt.subplot(133)  # 选择第二个子图
        plt.imshow(img, cmap='gray')
        # plt.scatter(point_loc[1], point_loc[0], color='red', marker='o')
        plt.title(f'{text} : {score}')               
        # 调整子图布局
        plt.tight_layout()            
        # 保存图像
        save_dir = f'image/{mode}/{epoch}/{text}/{case_num}/'
...
def cal_intersection_union(y_pred, y_true, th=0.5):
    binary_y_true = (y_true > th).float()
    sigmoid_pred = torch.sigmoid(y_pred)
    binary_y_pred = (sigmoid_pred > th).float()
    # print('ccc', torch.sum(y_pred <= 0), torch.sum(y_pred > 0), torch.sum(binary_y_true > 0),)
    # if torch.sum(binary_y_pred).item() < 200 :
    #     return 0, torch.sum(binary_y_true), 0
    intersection = torch.sum(binary_y_true * binary_y_pred)
    union = torch.sum(binary_y_true) + torch.sum(binary_y_pred)
    # print('intersection', intersection, union)
    # dice = (2.0 * intersection) / (union + 1e-10)
    return intersection, union, torch.sum(binary_y_pred)
...
def freeze_model(model):
    for name, param in model.named_parameters():
        if name in unfreeze_layers or 'Adapter' in name:
            continue
...
def custom_load_decathlon_datalist(json_data_path, index):
    ...
    for i in range(len(target)):
        target[i]['image'] = args.data_path + target[i]['image'][1:]
        target[i]['label'] = args.data_path + target[i]['label'][1:]

    return target

# class ResizeIfSmaller(Transform):
#     def __init__(self, target_size, allow_larger=False):
#         self.target_size = target_size
#         self.allow_larger = allow_larger

#     def __call__(self, data):
#         depth = data.shape[-1]  # Assuming data is a torch tensor
#         print('depth', depth)
#         if depth >= self.target_size : 
#             return data
#         else:
#             print('data', data.shape)
#             resize_transform = ResizeWithPadOrCropd(spatial_size=(None, None, self.target_size))
#             print('resize_transform(data)', resize_transform(data).shape)
#             return resize_transform(data)


def get_train_transform(ct_min, ct_max, ct_mean, ct_std, patch, spacing):
    zscore_normalization = Lambda(lambda x: (x - ct_mean) / ct_std)
    return Compose(
        [   
...
def get_evaluation_transform(ct_min, ct_max, ct_mean, ct_std, spacing):
    zscore_normalization = Lambda(lambda x: (x - ct_mean) / ct_std)
    return Compose(
        [
            LoadImaged(keys=["image", "label"], ensure_channel_first=None),
            AddChanneld(keys=["image", "label"]),
            ScaleIntensityRanged(
                keys=["image"], a_min=ct_min, a_max=ct_max, b_min=0.0, b_max=1.0, clip=True
            ),
            CropForegroundd(keys=["image", "label"], source_key="image"),
            Orientationd(keys=["image", "label"], axcodes="RAS"),
            Spacingd(
...
def get_decath_loader(args):
    ...
    class_list = [6,7,9,10] # 2,3,6,7,8,9,10
    # combined_train_dataset = CacheDataset(
    #     data=[],
    #     transform=None,  
    #     cache_num=0,     
    #     cache_rate=0.0,
    #     num_workers=0,
    # )
    # combined_valid_dataset = CacheDataset(
    #     data=[],
    #     transform=None,  
    #     cache_num=0,     
    #     cache_rate=0.0,
    ...
    train_dataset_list = []
    valid_dataset_list = []
    train_eval_dataset_list = []
    valid_eval_dataset_list = []
    ...
    for i in range(len(class_list)) :
        ...
        split_index = len(datalist) * 3 // 10
        train_dataset = CacheDataset(
            data = datalist[:split_index],
            transform = get_train_transform(ct_min[task], ct_max[task], ct_mean[task], ct_std[task], patch_size[task], spacings[task]),
            cache_num = 24,
            cache_rate = 1.0,
            num_workers = 8,
        )
        valid_dataset = CacheDataset(
            data = datalist[split_index:split_index+10],
            transform = get_train_transform(ct_min[task], ct_max[task], ct_mean[task], ct_std[task], patch_size[task], spacings[task]),
            cache_num = 24,
            cache_rate = 1.0,
            num_workers = 8,
        )
        train_eval_dataset = CacheDataset(
            data = datalist[:5],
            transform = get_evaluation_transform(ct_min[task], ct_max[task], ct_mean[task], ct_std[task], spacings[task]),
            cache_num = 24,
            cache_rate = 1.0,
            num_workers = 8,
        )
        valid_eval_dataset = CacheDataset(
            data = datalist[split_index:split_index+5],
            transform = get_evaluation_transform(ct_min[task], ct_max[task], ct_mean[task], ct_std[task], spacings[task]),
            cache_num = 24,
            cache_rate = 1.0,
            num_workers = 8,
        ...
        train_eval_dataset_list.append(train_eval_dataset)
        valid_eval_dataset_list.append(valid_eval_dataset)
    ...
    #     num_workers=8,
    # )
    # train_ds = train_dataset_list[0] + train_dataset_list[1]
    concatenated_train_dataset = ConcatDataset(train_dataset_list)
    train_loader = ThreadDataLoader(concatenated_train_dataset, num_workers=0, batch_size=args.b, shuffle=True)
    # val_ds = CacheDataset(
    #     data=valid_list, transform=transforms, cache_num=6, cache_rate=1.0, num_workers=8
    # )
    # val_ds = valid_dataset_list[0] + valid_dataset_list[1]
    concatenated_valid_dataset = ConcatDataset(valid_dataset_list)
    val_loader = ThreadDataLoader(concatenated_valid_dataset, num_workers=0, batch_size=1, shuffle=False)
    ...
    #     num_workers=8,
    # )
    concatenated_train_eval_dataset = ConcatDataset(train_eval_dataset_list)
    concatenated_valid_eval_dataset = ConcatDataset(valid_eval_dataset_list)
    train_eval_loader = ThreadDataLoader(concatenated_train_eval_dataset, num_workers=0, batch_size=args.b, shuffle=True)
    # val_eval_ds = CacheDataset(
    #     data=valid_list, transform=eval_transforms, cache_num=6, cache_rate=1.0, num_workers=0
    # )
    val_eval_loader = ThreadDataLoader(concatenated_valid_eval_dataset, num_workers=0, batch_size=1, shuffle=False)

    set_track_meta(False)

    return train_loader, val_loader, train_eval_loader, val_eval_loader #, transforms, eval_transforms, train_list, valid_list
...
def get_train_dataset(args):
    ...
    file_name = '../numpy_data_slice/train/small/imgs.pkl'  
    with open(file_name, 'rb') as file:
        images = joblib.load(file)  
    # file_name = '../numpy_data_slice/train/large_with_neg_overall/imgs2.pkl'  
    # with open(file_name, 'rb') as file:
    #     images2 = joblib.load(file)  
    # images.extend(images2)
    # images2.clear()
    # file_name = '../numpy_data/train/labels.pkl'
    file_name = '../numpy_data_slice/train/small/labels.pkl'
    with open(file_name, 'rb') as file:
        labels = joblib.load(file)  
    # file_name = '../numpy_data_slice/train/large_with_neg_overall/labels2.pkl'
    # with open(file_name, 'rb') as file:
    #     labels2 = joblib.load(file)  
    # labels.extend(labels2)
    # labels2.clear()
    # file_name = '../numpy_data/train/texts.pkl'  
    file_name = '../numpy_data_slice/train/small/texts.pkl' 
    with open(file_name, 'rb') as file:
        texts = joblib.load(file)   
    # file_name = '../numpy_data_slice/train/large_with_neg_overall/texts2.pkl' 
    # with open(file_name, 'rb') as file:
    #     texts2 = joblib.load(file)   
    # texts.extend(texts2)
    # texts2.clear()、
    print(len(images), len(labels), len(texts))
    # train_imgs = images[0:210] + images[262:363] + images[388:838] + images[950:1435] +images[1556:1622] + images[1638:1840]
    # valid_imgs = images[210:262] + images[363:388] + images[838:950] + images[1435:1556] +images[1622:1638] + images[1640:1890]
    # train_labels = labels[0:210] + labels[262:363] + labels[388:838] + labels[950:1435] +labels[1556:1622] + labels[1638:1840]
    # valid_labels = labels[210:262] + labels[363:388] + labels[838:950] + labels[1435:1556] +labels[1622:1638] + labels[1640:1890]
    # train_texts = texts[0:210] + texts[262:363] + texts[388:838] + texts[950:1435] +texts[1556:1622] + texts[1638:1840]
    # valid_texts = texts[210:262] + texts[363:388] + texts[838:950] + texts[1435:1556] +texts[1622:1638] + texts[1640:1890] 
    # train_slice_list = [slice(0, 210, 1), slice(262, 362, 1), slice(388, 628, 1), slice(1256, 1496, 1), slice(1556, 1622, 1), slice(1638, 1840, 1)] 
    # valid_slice_list = [slice(210, 262, 1), slice(362, 388, 1), slice(628, 688, 1), slice(1496, 1556, 1), slice(1622, 1638, 1), slice(1840, 1890, 1)] 
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=args.b, num_workers=0, shuffle=True)
    valid_dataset = MultiLabelDataset(valid_imgs, valid_labels, valid_texts)
    valid_dataloader = DataLoader(valid_dataset, batch_size=args.b, num_workers=0, shuffle=False)   
...
def get_eval_dataset(args):
    ...
    file_name = '../numpy_data_slice/eval/overall_large/imgs.pkl'  
    with open(file_name, 'rb') as file:
        images = pickle.load(file) 


    file_name = '../numpy_data_slice/eval/overall_large/labels.pkl'  
    with open(file_name, 'rb') as file:
        labels = pickle.load(file) 


    file_name = '../numpy_data_slice/eval/overall_large/texts.pkl'  
    with open(file_name, 'rb') as file:
        texts = pickle.load(file)  

    eval_dataset = MultiLabelDataset(images, labels, texts)
    eval_dataloader = DataLoader(eval_dataset, batch_size=args.b, shuffle=False)
    return eval_dataloader

def get_btcv_eval_dataset(args):
    ## create dataloader
    class MultiLabelDataset(Dataset):
        def __init__(self, data, labels, texts):
            self.data = data
            self.labels = labels
            self.texts = texts  
    ...
    file_name = '../btcv_numpy_data/eval_overall/imgs.pkl'  
    with open(file_name, 'rb') as file:
        eval_images = pickle.load(file) 


    file_name = '../btcv_numpy_data/eval_overall/labels.pkl'  
    with open(file_name, 'rb') as file:
        eval_labels = pickle.load(file) 


    file_name = '../btcv_numpy_data/eval_overall/texts.pkl'  
    with open(file_name, 'rb') as file:
        eval_texts = pickle.load(file)  

    eval_dataset = MultiLabelDataset(eval_images, eval_labels, eval_texts)
    eval_dataloader = DataLoader(eval_dataset, batch_size=args.b, shuffle=False)
...
class DiceCoeff(Function):
    ...
    def forward(self, input, target):
        ...
        return t, self.inter, self.union
...
def dice_coeff(input, target):
    ...
    for i, c in enumerate(zip(input, target)):
        score, _, _ = DiceCoeff().forward(c[0], c[1])
...
def eval_seg(pred,true_mask_p,threshold):
    ...
    gt_vmask_p = (true_mask_p > threshold).float()
    sigmoid_pred = torch.sigmoid(pred)
    vpred = (sigmoid_pred > threshold).float()
    vpred_cpu = vpred.cpu()
    disc_pred = vpred_cpu[:,0,:,:].numpy().astype('int32')
    disc_mask = gt_vmask_p [:,0,:,:].squeeze(1).cpu().numpy().astype('int32')

    iou_score_list = iou(disc_pred,disc_mask)
    dice_score_list = dice_coeff(vpred[:,0,:,:], gt_vmask_p[:,0,:,:])
    '''iou for numpy'''
    eiou += iou_score_list.mean()
    '''dice for torch'''
    edice += (sum(dice_score_list)/len(dice_score_list)).item()

    for i in range(disc_mask.shape[0]):
        if np.sum(disc_mask[i]) > 0 :
            pos_dice_list.append(dice_score_list[i])
            pos_pred_area_list.append(np.sum(disc_pred[i]))
        else :
            neg_pred_area_list.append(np.sum(disc_pred[i]))  

    return (eiou , edice), pos_dice_list, pos_pred_area_list, neg_pred_area_list
...
#             msk_s = msk[j,:,:,i]
#             if pt_label > 0 :
#                 indices = torch.nonzero(msk_s == pt_label)
#             else :
#                 indices = torch.nonzero(msk_s)
#             if indices.size(0) == 0:
#                 # generate a random array between [0-h, 0-w]:
#                 pos_neg_s.append(torch.tensor(0))
#                 random_index = torch.tensor([0, 0]).to(device = msk.device)
#                 # random_h = torch.randint(0, h, (1,))
#                 # random_w = torch.randint(0, w, (1,))
#                 # random_index = torch.cat((random_h, random_w)).to(device = msk.device)
#                 # random_index = torch.randint(0, h, (2,)).to(device = msk.device)
#                 new_s = (msk_s == pt_label).to(dtype = torch.float)    
#             else:
#                 pos_neg_s.append(torch.tensor(1))
...
#         pos_negs = torch.stack(pos_neg_s, dim=0)
        
#         pt_list.append(pts)
#         msk_list.append(msks)
#         pos_neg_list.append(pos_negs)

#     pt = torch.stack(pt_list, dim=-1)
#     msk = torch.stack(msk_list, dim=-1)
#     pos_neg = torch.stack(pos_neg_list, dim=-1)

#     msk = msk.unsqueeze(1)

#     return img, pt, msk, pos_neg #[b, 2, d], [b, c, h, w, d]
def find_extreme_coords(data):
    ...
    if len(coords) > 0:
        ...
        min_coord = min_coord[1:3].tolist()
        max_coord = max_coord[1:3].tolist()
    else :
        min_coord = [-1,-1]
        max_coord = [-1,-1]
...
def generate_click_prompt(img, msk, pt_label = -1):
    ...
    pos_neg = torch.stack(pos_neg_list, dim=-1)
    bbox_list = []
    min_coord, max_coord = find_extreme_coords(msk.cpu().numpy())
    for s in range(pos_neg.shape[1]):
        if s >= start_slice and s <= end_slice:
            bbox = torch.Tensor([min_coord, max_coord])
        else :
            bbox = torch.Tensor([[-1, -1], [-1, -1]])
        bbox_list.append(bbox)
    bboxes = torch.stack(bbox_list)
    # print('bboxes', bboxes.shape, start_slice, end_slice, min_coord, max_coord, bboxes)
    msk = msk.unsqueeze(1)
    # print('final shape', img.shape, msk.shape, pt.shape, pos_neg.shape)

    return img, pt, msk, pos_neg, bboxes #[b, 2, d], [b, c, h, w, d]
